		...
		def LoadFile():
			...
			for line in lines:
				if line[0] == "x":
					...
					parsed = line.split(" ")
					self.patterns[j].WriteDuration(int(parsed[2]))
					self.patterns[j].WriteCount(int(parsed[4]))
					self.patterns[j].WriteStatus(int(parsed[6]), "ON")

					# Copied from below. No idea what it does?!
					while not self.patterns[j].GetStatus(k):
						k += 1
						if k == 64:
							break

					self.patterns[j].WriteIntensity(k, int(parsed[8]))
